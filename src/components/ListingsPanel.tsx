...
export default function ListingsPanel({ city, district, offerType, filters, sortBy: sortByProp, onSortChange, onExpand, onListingHover, onClose, ignoredListings, favouriteListings, onIgnore, onFavourite }: ListingsPanelProps) {
  const [listings, setListings] = useState<Listing[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadingMore, setLoadingMore] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState<ListingFilter>('all');
  const [sortByLocal, setSortByLocal] = useState<SortBy>('price_m2_asc');
  const sortBy = sortByProp !== undefined ? sortByProp : sortByLocal;
  const setSortBy = onSortChange ?? setSortByLocal;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Prominent expand button — this is a flagship feature, make it visible */}
      {onExpand && (
        <button
          onClick={onExpand}
          title="Switch to full-width list view"
          className="mx-3 mt-2 px-3 py-2 rounded flex items-center justify-center gap-2 border border-[#00d4aa40] bg-[#00d4aa15] text-[#00d4aa] hover:bg-[#00d4aa] hover:text-black hover:border-[#00d4aa] transition-colors font-mono text-[11px] font-semibold tracking-wider flex-shrink-0"
        >
                  ...
                  {listing.thumbnailUrl && (
                    <div className="flex-shrink-0">
                      <a href={listing.url} target="_blank" rel="noopener noreferrer">
                        <img
                          src={morizonPhotoAtSize(listing.thumbnailUrl, 's')}
                          alt=""
